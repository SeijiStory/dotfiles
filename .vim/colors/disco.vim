...
fun! <SID>set_colors(group, fg, bg, attr)
    ...
    if !empty(a:fg)
        if type(a:fg) == 1 " If a:bg is a string, look it up for cterm
            exec "hi " . a:group . " ctermfg=" . get(s:color_map, a:fg)
        else
    ...
    if !empty(a:bg)
        if type(a:bg) == 1 && a:bg != 'NONE' " If a:bg is a string, look it up for cterm
            exec "hi " . a:group . " ctermbg=" . get(s:color_map, a:bg)
        else
    ...
    if a:attr != ""
        exec "hi " . a:group . " cterm=" . a:attr . " term=" . a:attr
    endif
...
call <SID>set_colors("ModeMsg"    , s:fg           , ""      , "")
call <SID>set_colors("MoreMsg"    , s:fg           , ""      , "")
...
call <SID>set_colors("StatusLine"       , s:bg       , s:dimtwo       , "reverse,bold")
call <SID>set_colors("StatusLineNC"     , s:bg       , s:dimtwo       , "reverse,bold")
call <SID>set_colors("StatusLineTerm"   , s:bg       , s:dimtwo       , "reverse,bold")
call <SID>set_colors("StatusLineTermNC" , s:bg       , s:dimtwo       , "reverse,bold")
...
call <SID>set_colors("Constant", s:magenta, "", "")
...
call <SID>set_colors("Statement", s:yellow, "", "")
